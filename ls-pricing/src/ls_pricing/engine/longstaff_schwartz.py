...
class LongstaffSchwartzEngine:
    ...
    def price_american_option(
        ...
        for i in range(len(exercise_steps) - 1, -1, -1):
            ...
            if np.sum(itm_mask) > 10:  # Need enough points for regression
                ...
                future_cf_pv = self._discount_future_cashflows(
                    cash_flows, discount_factors, int(step)
                )
        ...
        mean_exercise_time = (
            np.mean(exercise_time[exercise_time > 0]) if exercise_prob > 0 else 0.0
        )
    ...
    def _calculate_payoff(
        self, bond_prices: np.ndarray, strike: float, option_type: str
    ) -> np.ndarray:
    ...
    def _discount_future_cashflows(
        self, cash_flows: np.ndarray, discount_factors: np.ndarray, current_step: int
    ) -> np.ndarray:
        ...
        future_indices = (
            np.where(np.any(cash_flows[:, current_step + 1 :] > 0, axis=0))[0]
            + current_step
            + 1
        )
    ...
    def _calculate_option_values(
        self, cash_flows: np.ndarray, discount_factors: np.ndarray
    ) -> np.ndarray:
