...
class TestCouponBond:
    def setup_method(self):
        ...
        self.bond = CouponBond(
            face_value=100.0, coupon_rate=0.05, maturity=10.0, payment_frequency=2
        )
...
class TestCallableBondEngine:
    ...
    def test_issuer_perspective(self):
        ...
        investor_result = self.cb_engine.price_callable_bond(
            self.callable_bond, from_investor_perspective=True
        )

        # From issuer perspective
        issuer_result = self.cb_engine.price_callable_bond(
            self.callable_bond, from_investor_perspective=False
        )

        # Prices and probabilities should be the same
        assert (
            investor_result["callable_bond_price"]
            == issuer_result["callable_bond_price"]
        )
        assert (
            investor_result["exercise_probability"]
            == issuer_result["exercise_probability"]
        )

        # Option values should have opposite signs
        assert np.isclose(
            investor_result["option_value"], -issuer_result["option_value"], rtol=1e-6
        )
    ...
    def test_no_call_before_lockout(self):
        ...
        mc_engine_short = MonteCarloEngine(
            model=self.hw_model, n_paths=1000, n_steps=20
        )
